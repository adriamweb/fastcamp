#inicialmente faz as importações necessárias
from google.adk.agents import Agent

from google.adk.tools.tool_context import ToolContext

# comentários feitos por mim:
# uma ferramenta que retorna uma piada nerd sobre um tópico especififico. 
# a função recebe o tópico e o tool_context para armazenar no estado da sessão.
def get_nerd_joke(topic: str, tool_context: ToolContext) -> dict:
    ...
    # Comentário feito por mim: busca a piada pelo tópico, usando default caso não encontre.
    joke = jokes.get(topic.lower(), jokes["default"])

    # Update state with the last joke topic
    # Comentário feito por mim: salva o último pedido no estado da sessão,
    # para que outros agentes ou ferramentas consigam recuperar a informação
...
# Comentário feito por mim: cria o subagente que tem como especialidade contar piadas nerds, 
# sendo sub_agent passado pro manager e usando a ferramenta get_nerd_joke criada anteriormente
funny_nerd = Agent(
    name="funny_nerd",
